...
void printRowBuffer(){
    for(int i=0; i<Row_Buffer.size(); i++){
        if(Row_Buffer[i] != 0){
            cout << "index " << i << " : value " << Row_Buffer[i] << ", ";
        }
    }
    cout << endl;
}
...
void init(string infilename)
{
    ...
    while (getline(inputfile, x))
    {
        ...
        if (x.find(':') != -1)
        {
            ...
            tempLabel.erase(std::remove_if(tempLabel.begin(), tempLabel.end(), ::isspace), tempLabel.end());
...
void parseInstruction(LinkedList *l, string currentInstruction)
{
    ...
    else if (op == "beq" || op == "bne")
    {
        ...
        if (!findMyRegister(rs1) || !findMyRegister(rs2))
        {
            cout << "Error in parsing" << endl;
            exit(1);
        }
    ...
    else if (op == "lw" || op == "sw")
    {
        ...
        insertAtTail(l, op, rd, address, "-1");
    }
...
void executer(Node *node)
{
    ...
    {
        r[0] = giveIndexOfMyRegister(r_0);
        r[1] = giveIndexOfMyRegister(r_1);
        r[2] = giveIndexOfMyRegister(r_2);
        add();
    }
    else if (op == "sub")
    {
...
void modifier(Node *node) // incomplete
{
    if (node == NULL)
    {
        return;
    }
    if (node->instruction != "lw" && node->instruction != "sw")
    {
        modifier(node->next);
        return;
    }
    int rowNumber = stoi(node->r1) / 1024; // here we store rownumber used for give node
    Node *curr = node->next;
    unordered_set<string> usedReg;
    unordered_set<string> modifiedReg;
    int rowNumber2;
    while (curr != NULL)
    {
        if (curr->instruction == "lw" || curr->instruction == "sw")
        {
            rowNumber2 = stoi(curr->r1) / 1024;
    ...
    if (curr->instruction == "sw")
    {
        usedReg.insert(curr->r0);
    }
    else
    {
        modifiedReg.insert(curr->r1);
    }
    Node *curr2 = curr->next;
    bool check = true;
    while (curr2 != NULL)
    {
        if (curr2->instruction == "lw" || curr2->instruction == "sw")
        {
            rowNumber2 = stoi(curr2->r1) / 1024;
            if (rowNumber2 == rowNumber)
            {
                if (curr2->instruction == "sw")
                {
                    check = (modifiedReg.find(curr2->r0) == modifiedReg.end());
                }
                else
                {
                    check = (usedReg.find(curr2->r0) == usedReg.end());
                }
                if (check)
                {
                    modifier(move(curr, curr2));
                }
                else
                {
                    modifier(curr);
        ...
        string op = curr2->instruction;
        if (op == "j" || op == "bne" || op == "beq")
        {
            modifier(curr);
            return;
        }
        else if (op == "add" || op == "sub" || op == "mul" || op == "slt")
        {
            usedReg.insert(curr2->r1);
            usedReg.insert(curr2->r2);
            modifiedReg.insert(curr2->r0);
        }
        else if (op == "addi")
        {
            usedReg.insert(curr2->r1);
...
int main()
{
    ...
    CLK = 0;
    cout << '\n';
    cout << "Order in which instructions are executed: " << endl;
    cout << "---------------------------------------------" << endl;
    for (int i = 0; i < lists.size(); i++)
    {
        modifier(lists[i]->head);
        printList(lists[i]);
    }
    cout << "---------------------------------------------" << endl;
    cout << endl;
